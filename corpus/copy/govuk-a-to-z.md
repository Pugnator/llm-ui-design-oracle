<!-- source: https://www.gov.uk/guidance/style-guide/a-to-z-of-gov-uk-style
     ...
     title: GOV.UK style guide — A to Z (includes words to avoid) -->

Skip past section navigation
Sign up to receive email updates about changes to the GOV.UK content and publishing guidance including the style guide.
...
Always use acronym code so the full explanation is available as hover text.
...
There are other abbreviations you do not need to explain in technical contexts. Read the technical A to Z style guide for more information.
...
Only use title case when referring to the name of an academy, like Mossbourne Community Academy. Look at the entry on titles for more on this.
...
Read the guidance about using clear language for more details on using the active voice.
...
Title case when referring to the national Adoption Register.
...
Title case when referring to the Agile Manifesto and principles and processes, otherwise use lower case.
...
Use numbered steps instead of bullet points to guide a user through a process. You do not need a lead-in line and you can use formatting like links and attachments in steps.
...
- specific job titles - see the entry on job titles
...
- government - see the entry on government
...
Lower case in text. Title case in titles: Spencer Tracy, Chairman, GDS.

### Change notes

See the guidance on writing change notes.
...
Title case as it represents a single court system.
...
See the ‘Writing about ethnicity’ guidance for more details.
...
See the entry on eg, etc and ie.
...
Lower case in text. Title case in titles: Spencer Tracy, Executive Director, GDS.
...
See also the entry for the GOV.UK One Login app.
...
See also the entry on sign in or log in.
...
Title case for names of groups, directorates and organisations: Knowledge and Innovation Group.
...
Use ‘to’ for time ranges and date ranges, not hyphens.
...
See also the entry on Shadow job titles.
...
Title case when referring to The Kanban Method, otherwise lower case.
...
Title case if used as the full title: the National Assembly for Wales (Legislative Competence) (Social Welfare) Order 2008.
...
Lists should be bulleted to make them easier to read. See the entry on bullets and steps.
...
When referring to local government, use local council instead of local authority where possible. See also the entry on council.
...
See also the entries on sign in or log in and GOV.UK One Login.
...
See the entry on words to avoid.
...
Use local council, instead of local authority, where possible. See also the entry on council.
...
All content on GOV.UK should be written in plain English. You should also make sure you use language your audience will understand - check the entry for words you should avoid.
...
Use block quotes for quotes longer than a few sentences.
...
Corallo AN and others. ‘A systematic review of medical practice variation in OECD countries’ Health Policy 2014: volume 114, pages 5 to 14 (viewed on 18 November 2014).
...
See the entry on national curriculum tests.

### School Admissions Code

Title case. After the first mention you can refer to it in lower case: the admissions code or the code.
...
Title case when referring to the framework and method for developing products, otherwise use lower case.
...
Title case when referring to government security classifications, otherwise lower case.
...
Title case. Write in full the first time you use it, then use the Senedd.
...
See also the entry on Job titles.
...
See the entry on junk and spam emails.
...
Title case. When not using the full title in subsequent mentions, refer to it in lower case: the code of practice or the code.
...
See the entry on quotes and speech marks.
...
Read the Office for National Statistics (ONS) style guide to find out how to write about statistics. This has been produced by the Office for National Statistics for all members of the Government Statistical Service.
...
See the entry on bullet points and steps.
...
- be frontloaded and optimised for search
- use a colon to break up longer titles
- not contain dashes or slashes
- not have a full stop at the end
- not be questions
- explain abbreviations unless they are well known - see the entry on abbreviations
...
The social media platform is now known as X - see the entry on X.
...
Lower case for online service.

### VAT on e-Services

Uppercase S in e-Services.
...
Read more about using plain English.
...
- Share by  Email  (opens in new tab)
- Share on  Microsoft Teams  (opens in new tab)
