<!-- source: https://learn.microsoft.com/en-us/windows/win32/uxguide/ctrl-tooltips-and-infotips
     fetched: 2026-09-19
     licence: CC BY 4.0 — https://github.com/MicrosoftDocs/win32/blob/docs/LICENSE
     title: Win32 UX Guide — Tooltips and Infotips -->
...
[image: Screenshot that shows print button with the tooltip 'Print (Ctrl+P)' displayed.]
...
[image: screen shot of share button with infotip]
...
[image: screen shot of print button with tooltip]
...
| Usage | Example |
|---|---|
| **Tooltips**  display the label of an unlabeled control or glyph. | Because these tips serve as labels, their text follows the label guidelines for the underlying control.   [image: screen shot of export list button with tooltip]  in this example, the tooltip gives the command label.  [image: screen shot of close button with tooltip][image: screen shot of play button with tooltip]  in these examples, tooltips label graphic buttons.  [image: screen shot of show menu glyph with tooltip]  In this example, the tooltip labels a glyph. |
| **Infotips**  provide a supplemental description or explanation of an object or control. | Use infotips to describe or explain objects and controls such as toolbar controls, icons (including icon overlays), links, tabs, progressive disclosure controls, and custom controls.   [image: screen shot of e-mail button with infotip]  [image: screen shot of burn button with infotip]  In these examples, infotips provide supplemental information about controls and objects. |
| **Alt text infotips**  describe a graphic for accessibility. | This pattern is primarily for users who have some form of vision impairment, and may be using a screen reader.   [image: screen shot of windows start button with infotip]  In this example, the infotip describes the Start menu graphic. |
| **Thumbnails**  display a small image of an item. | Thumbnails give an easily recognizable graphic representation of a window or document.   [image: screen shot of control panel categories thumbnail]  in this example, the windows taskbar gives thumbnail tips for its items.  [image: screen shot of thumbnail of photo and its data]  In this example, Windows Photo Gallery gives thumbnail tips for its items. |
| **Detail infotips**  display detailed information about an object. | Infotips are an effective way to show detailed information about an object, or to provide data.   [image: screen shot of photo's infotip showing file type][image: screen shot of graph with infotip detailing values]  In these examples, infotips give detailed information about an object or data. |
| **Start Menu infotips**  describe an item on the start menu. | The start menu consists of program names and important windows destinations, such as documents, pictures, and control panel. these tips describe start menu items, typically by giving a brief description of the program or destination as well as the primary tasks users can perform with it. these descriptions are also indexed by the start menu search box, further helping users find the programs they need.   [image: screen shot of welcome center infotip]  In this example, the infotip describes what users can do with a program in the Start menu. |
| **Control Panel infotips**  describe a control panel category or task. | These tips provide supplemental information to help users choose the right control panel category and item.   [image: screen shot of user accounts category with infotip]  In this example, the infotip describes the User Accounts Control Panel category. |
| **Full name infotips**  display the full name of an item when the name is truncated or not fully visible. | These tips allow you to display items in a more compact space, while reducing the need for horizontal scrolling. this is especially important when the content length is unknown because it is dynamic. unlike the other patterns, when used in lists and trees these tips are displayed directly over the source object.   [image: screen shot of radio-button group-title infotip]  In this example, an infotip is used to display the full item name on hover. |
| **Status infotips**  display status information for notification area icons. | Normally, tips should be static because users don't expect them to change from one instance to the next. **notification area icons are an exception** because these icons communicate status, and there is no other screen space available for status text.   [image: screen shot of 'messenger not signed in' infotip]  [image: screen shot of 'messenger signed in' infotip]  In these examples, infotips give status information for notification area icons. |
...
- **Avoid covering the object the user is about to view or interact with.** Always place the tip on the side of the object, even if that requires separation between the pointer and the tip. Some separation isn't a problem as long as the relationship between the object and its tip is clear.  **Incorrect:** [image: screen shot of infotip obscuring search box] **Correct:** [image: screen shot of infotip placed below search box] In the correct example, the infotip is placed away from the Search box, even though that requires space between it and the caret. **Incorrect:** [image: screen shot of infotip obscuring revised text] **Correct:** [image: screen shot of infotip placed above revised text] In the correct example, the underlying text is far more useful than the tip, so the infotip is placed well out of the way.
  - **Exception:** Full name tooltips used in lists and trees.

- **For collections of items, avoid covering the next object that the user is likely to view or interact with.** For horizontally arranged items, avoid placing tips to the right; for vertically arranged items, avoid placing tips below. **Incorrect:** [image: screen shot of infotip obscuring recent items list] **Correct:** [image: screen shot of infotip beside recent items list] In the incorrect example, the tip covers the object the user is most likely to interact with next.
...
- As with normal labels, **keep tooltips brief** typically five words or less but prefer specific labels over vague ones. **Acceptable:** [image: screen shot of print tooltip] **Better:** [image: screen shot of 'print to default printer' tooltip] **Best:** [image: screen shot of 'print (document writer)' tooltip] **Incorrect:** [image: screen shot of verbose tooltip] In these examples, the best example is both concise and specific, whereas the incorrect example is unnecessarily verbose.
- **Tooltips may also provide more detail for labeled toolbar buttons if doing so is helpful.** Don't just repeat or give a wordy restatement of what is already in the label. **Correct:** [image: screen shot of 'search all outlook' tooltip] In this example, the tooltip explains what Search does. **Incorrect:** [image: screen shot of tooltip repeating button label] In this example, the tooltip just repeats what is already in the label.
- **You don't have to give labeled controls tooltips simply for the sake of consistency.** [image: screen shot of labeled and unlabeled buttons] In this example, the unlabeled toolbar buttons have tooltips, but the labeled ones don't.
- Whenever appropriate, **make tooltips more helpful by providing keyboard shortcuts and default values.** Put this additional information in parentheses. Doing so makes tooltips helpful for labeled controls even when they otherwise just repeat the label. Don't consider this additional text when evaluating the conciseness of a tooltip. [image: screen shot of 'print (document writer)' tooltip] In this example, Word displays default values and keyboard shortcuts in the toolbar tooltips.
...
- **Format infotips to make their content easier to read and scan.** Large blocks of unformatted text can be difficult to read. **Incorrect:** [image: screen shot of long, unstructured, run-on tooltip] **Correct:** [image: screen shot of same tooltip with one line per item] In the correct example, the formatted text is much easier to read and scan.
...
- **Start with a present-tense, imperative verb** such as create, edit, show, and send. Prefer specific verbs over generic verbs such as manage and open, which really apply to most Start menu items. Get right to the point. **Incorrect:** [image: screen shot of tooltip: opens the music folder] **Better:** [image: screen shot of tooltip: store and play music] In the incorrect example, the infotip starts with a generic verb. The better example gets right to the point with specific verbs, but continues to use the unnecessary "and other" phrasing at the end of the tip.
- **Don't use language that sounds like marketing.** **Incorrect:** [image: screen shot of 'one-stop starting point' infotip] In this example, the infotip sounds like marketing.
- Because these infotips are indexed for the Start menu search box, **describe your program's important tasks using terms for which users are most likely to search. Consider using keywords and common synonyms.** **Incorrect:** [image: screen shot of tooltip: create dvds] **Correct:** [image: screen shot of tooltip: create (burn) cds, dvds] In the correct example, the infotip has common synonyms.
- Use sentence-style capitalization.
...
- **Be specific.** Avoid generic verbs and catch-all phrases like and other hardware. If the information is important, list it specifically; otherwise, assume that users understand that not everything is listed in the infotips. **Incorrect:** [image: screen shot of tooltip: configures your mouse] **Correct:** [image: screen shot of detailed tooltip for mouse settings] In the correct example, the types of hardware configured are specifically listed.
- **Be concise.** Use 25 words or less. Longer infotips discourage reading.
- **Start with a present-tense, imperative verb.** **Correct:** Configure Internet display and connection settings. Adjust settings for vision, hearing, and mobility.
...
- Don't use language that sounds like marketing. **Incorrect:** Your one-stop starting point for all your disk configuration needs.
- Because these infotips are indexed for the Control Panel search box, **describe items using terms for which users are most likely to search.** Consider using common synonyms for popular tasks and objects. [image: screen shot of tooltip with game-controller tasks] In this example, the item is described using terms for which users are most likely to search.
- If a Control Panel item is likely to be confused with others, explain how it is different in the infotip. **Incorrect:** [image: screen shot of infotip lacking specific details] In this example, both Control Panel items configure sound, but the infotip doesn't clarify the difference. **Correct:** [image: screen shot of infotip with specific details] In this example, the difference between the two items is more evident because of the tip.
...
- For infotips, use icons only if they aid in recognition or comprehension, or provide context. Most infotips shouldn't have icons. [image: screen shot of volume infotip with headphone icon] In this example, the infotip has an icon to help associate the icon with its meaning.
