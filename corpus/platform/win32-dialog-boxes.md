<!-- source: https://learn.microsoft.com/en-us/windows/win32/uxguide/win-dialog-box
     fetched: 2026-09-19
     licence: CC BY 4.0 — https://github.com/MicrosoftDocs/win32/blob/docs/LICENSE
     title: Windows 7 Dialog Boxes (Design basics) - Win32 apps
     ms.date: 2022-01-25 -->
...
[image: screen shot identifying dialog box elements]
...
[image: screen shot of a typical task dialog box]
...
[image: screen shot of a typical task pane]
...
[image: Screenshot that shows a menu and sub-menu.]

[image: screen shot of a menu and submenu]
...
- **Don't have a menu bar or status bar.** Instead, provide access to commands and status directly on the dialog box itself, or by using context menus on the relevant controls.  **Incorrect:** [image: screen shot of a dialog box with a menu bar] In this example, Find Certificates is a modeless dialog box with a menu bar.
...
[image: screen shot of a dockable, modeless dialog box]
...
[image: screen shot of a progress bar]

[image: screen shot of 'no problems found' message]
...
[image: diagram of dialog box centered on window behind it]
...
[image: diagram of dialog box offset down and to the right]
...
[image: screen shot of centered media player buttons]
...
- **Whenever possible, assign unique access keys to all interactive controls or their labels.**Read-only text boxes are interactive controls (because users can scroll them and copy text) so they benefit from access keys. **Don't assign access keys to:**
  - **OK, Cancel, and Close buttons.** Enter and Esc are used for their access keys. However, always assign an access key to a control that means OK or Cancel, but has a different label. [image: screen shot of delete file dialog box] In this example, the positive commit button has an access key assigned.
...
[image: screen shot of copy dialog box with progress bar]
...
[image: screen shot of dialog box with cancel button]
...
[image: screen shot of dialog box with try again button]
...
[image: screen shot of progress bar and try again button]
...
[image: screen shot of dialog box with a large graphic]
...
[image: screen shot of dialog box with warning icon]
...
[image: screen shot of favorites dialog box with star icon]
...
[image: screen shot of dialog box with powerpoint icon]
...
[image: images of arrows showing how to position monitor]
...
[image: screen shot of about dialog box with windows logo]
...
[image: screen shot of dialog box with footnote icon]
...
| **Choice dialogs** | Modal dialogs: OK/Cancel or [Do it]/CancelModeless dialogs: Close button on dialog box and title barTask pane: Close button on title bar |
...
- **Don't confirm commit buttons.** Doing so unnecessarily can be very annoying. **Exceptions:**  For more guidelines and examples, see Confirmations.
...
- **Right-align commit buttons in a single row across the bottom of the dialog box,** but above the footnote area. Do this even if there is a single commit button (such as OK). **Incorrect:** [image: screen shot of message with centered ok button] In this example, the OK button is incorrectly centered.
...
- **Use positive commit buttons that are specific responses to the main instruction, instead of generic labels such as OK or Yes/No.** Users should be able to understand the options by reading the button text alone. **Exceptions:**
  ...
  - Certain tasks require more thought and careful reading for users to make informed decisions. This is usually the case with confirmations. **In such cases, you can purposely use generic commit button labels to force users to read the main instructions and prevent hasty decisions.** **Correct:** [image: screen shot of message with yes and no buttons] In this example, using Yes/No commit buttons forces users to at least read the main instruction.

- **Alternatively, you can add the word "anyway" to the positive commit button label to indicate that the dialog box presents a reason not to proceed** and that users should read the dialog carefully before proceeding. **Correct:** [image: screen shot of message and uninstall anyway button] In this example, "anyway" is added to the commit button label to indicate that users should proceed carefully.
...
- **Don't map generic labels to their specific meaning with text in the content area.** Instead, use specific commit button labels, or a question dialog using links if the labels are lengthy. **Incorrect:** [image: screen shot of message with unclear use of buttons] In this example, OK is mapped to Continue, Cancel is mapped to Remain on Page.
...
- **Use Yes and No buttons only to respond to yes or no questions.** The main instruction should be naturally expressed as a yes or no question. Never use OK and Cancel for yes or no questions. **Incorrect:** [image: Screenshot that shows a message with an 'OK' for a yes-no question.] **Correct:** [image: screen shot of message with yes for same question] **Better:** [image: screen shot of message with run for same question] In these examples, Yes and No are good responses to yes and no questions, but specific responses are even better.
- **Consider phrasing the main instruction as a yes or no question if commit buttons with specific phrasing turn out to be long or awkward.** Alternatively, you can use command links for longer responses (five words or more) to the main instruction. **Incorrect:** [image: screen shot of message with wordy button labels] **Correct:** [image: screen shot of message with yes/no button labels] The specific phrasing in the incorrect example is too long, so the correct example uses Yes and No.
...
- **Don't use OK buttons for errors or warnings.** Problems are never OK. Use Close instead. **Incorrect:** [image: screen shot of message with ok button] In this example, Close should be used instead of OK.
...
- **Provide a Cancel button to let users explicitly abandon changes.** Dialog boxes need a clear exit point. Don't depend on users finding the Close button on the title bar.  **Incorrect:** [image: screen shot of message with ok button only] In this example, having only a Close button on the title bar makes it appear as though users don't have a choice.
...
- **Don't use Cancel buttons to respond to questions.** **Incorrect:** [image: screen shot of message with ok for yes-no question] In this example, OK and Cancel are incorrectly used to respond to a Yes or No question.
...
- **If users need to both cancel the dialog box and the task, give commit buttons to do both.** Label the button that cancels the dialog box with a negative response to the main instruction. Label the button that cancels the entire task with Cancel. Using Cancel allows the dialog box to be used in many contexts. **Correct:** [image: screen shot of dialog box with save/don't save] In this example, this dialog box is displayed by Windows Paint as the result of a New or Exit command when the graphic hasn't been saved. Don't Save closes the dialog without saving, whereas Cancel cancels the New or Exit command. **Incorrect:** [image: screen shot of dialog box with yes/no buttons] In this example, there is no way to cancel the task (closing Office Shortcut Bar) that led to displaying this dialog box. This dialog box needs a Cancel button.
- **If users just need to cancel the dialog but not the task, use a button with a specific, negative response to the main instruction,** and don't have a Cancel button. [image: screen shot of dialog box with run/don't run] In this example, this dialog box is displayed indirectly as the result of navigating to a Web page that installs an ActiveX control. Using Cancel would be ambiguous here, so Don't run is used instead.
...
[image: screen shot of dialog box with text noting options]
...
[image: screen shot of dialog box with don't exit link]
...
[image: screen shot of message asking unnecessary question]
...
- **Incorrect:**
- [image: screen shot of message with ask me later option]
...
[image: screen shot of dialog box with more options button]
...
[image: screen shot of dialog box with clarifying footnote]
...
[image: screen shot of dialog box with dimmed controls]
...
- To indicate that users must provide information in a control, consider the following options:
  ...
  - **Indicate required input using an asterisk at the beginning of the label.** Explain the asterisk using either:  This approach works well if there aren't many required controls, but poorly if most controls are required. [image: screen shot of text box labels with asterisks] In this example, asterisks are used to indicate required input.
...
- **Locate Help links at the bottom of the content area of the dialog box.** If the dialog box has a footnote and the Help link is related to it, place the Help link within the footnote. [image: screen shot of dialog box with help link] In this example, the Help link applies to the entire dialog.
...
[image: screen shot of print dialog box]
...
[image: screen shot of text box with redundant label]

In this example, the text box label is just a restatement of the main instruction.

**Better:**

[image: screen shot of same text box with one label]
